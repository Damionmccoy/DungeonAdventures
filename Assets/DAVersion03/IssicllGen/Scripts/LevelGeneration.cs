...
public class LevelGeneration : MonoBehaviour {
	...
	void Start () {
        ...
        PlaceRoomsOfInterest();//selects the room to place the boss
    ...
    void PlaceRoomsOfInterest()
    {
        List<Room> endRooms = new List<Room>();

        foreach(Vector2 v in takenPositions)
        {
            if (NumberOfNeighbors(v, takenPositions ) == 1){
                foreach (Room room in rooms)
                {
                    if(room != null && room.gridPos == v)
                    {
                        endRooms.Add(room);
                    }
                }
        ...
        int rnd = Random.Range(0, endRooms.Count);
        int index = 0;
